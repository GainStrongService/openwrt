...
static int simple_parse_of(struct asoc_simple_priv *priv)
{
	...
	dev_err(dev, "simple_parse_of\n");
	if (!top)
		return -EINVAL;

	dev_err(dev, "simple_parse_of22\n");
	ret = asoc_simple_parse_widgets(card, PREFIX);
	if (ret < 0)
		return ret;

	dev_err(dev, "simple_parse_of33\n");
	ret = asoc_simple_parse_routing(card, PREFIX);
	if (ret < 0)
		return ret;

	dev_err(dev, "simple_parse_of44\n");
	...
	for (li.cpu = 1; li.cpu >= 0; li.cpu--) {
		...
		ret = simple_for_each_link(priv, &li,
					   simple_dai_link_of,
					   simple_dai_link_of_dpcm);
	dev_err(dev, "simple_parse_of55, %d\n", ret);
		if (ret < 0)
			return ret;
	}

	dev_err(dev, "simple_parse_of66\n");
	ret = asoc_simple_parse_card_name(card, PREFIX);
	if (ret < 0)
		return ret;

	dev_err(dev, "simple_parse_of77\n");
				  ...
				  struct link_info *li)
{
	...
	dev_err(dev, "simple_get_dais_count\n");
	/*
...
static int asoc_simple_probe(struct platform_device *pdev)
{
	...
	dev_err(dev, "Start probe simple card\n");
	...
	dev_err(dev, "asoc_simple_init_priv\n");
	...
	dev_err(dev, "devm_snd_soc_register_card\n");
